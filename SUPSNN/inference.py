...
def f1(synapse, train, var_D, m, j, t):
    train = tf.squeeze(train)         #(16384, 71)
    bb = tf.matmul(tf.reshape(synapse[j, :], (1, m)), tf.reshape(train[:, t], (m, 1)))  #输入图像所引导的脉冲序列直接加载到首层神经元上，得到当前第j个输出层神经元的当前时刻t的电压
    bb_new = tf.reshape(tf.math.subtract(bb, var_D), ())    #减去漏电常量
    if j == 0:
...
def f5_1(ss, LAYER2T_P):
    if ss == 0:
        ...
        sub = tf.stack([0.0, 500.0 * 0.001, 500.0 * 0.001], 0)
    if ss == 1:
        mult = tf.stack([1.0, 0.0, 1.0], 0)
        sub = tf.stack([500.0 * 0.001, 0.0, 500.0 * 0.001], 0)
    if ss == 2:
        mult = tf.stack([1.0, 1.0, 0.0], 0)
        sub = tf.stack([500.0 * 0.001, 500.0 * 0.001, 0.0], 0)
...
def Lateral_Inhibition(LAYER2T_P, LAYER2T_T, t_ref, ss):
    LAYER2T_P = f5_1(ss, LAYER2T_P)
    LAYER2T_T = f5_2(ss, LAYER2T_T, t_ref)
    return LAYER2T_P, LAYER2T_T
